...
// 맵 초기 세팅 함수
void fillMAP(const int& row, const int& column) {
	char c;
	for (int i = 1; i <= row; i++) {
...
// 디버깅용 출력 함수
void printMAP(const int& dimension, const int& row, const int& column) {
	cout << "\n";
	for (int i = 1; i <= row; i++) {
		for (int j = 1; j <= column; j++) {
...
// player BFS
void PBFS(queue<pair<int, int>> &q, const int& row, const int& column) {
	while (!q.empty()) {
		int y, x;
		y = q.front().first;
		x = q.front().second;
...
// fire BFS
void FBFS(queue<pair<int, int>>& q, const int& row, const int& column) {
	while (!q.empty()) {
		int y, x;
		y = q.front().first;
		x = q.front().second;
...
// 시작점이 탈출구인지 반환하는 함수
bool isBoundary(const int& row, const int& column) {
	for (int i = 1; i <= row; i++) {
